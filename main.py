...
# --- 1. SETTINGS & CSS ---
st.set_page_config(page_title="Titan V3", page_icon="🦅", layout="wide")
...
st.markdown("""
    ...
    /* Buttons */
    .stButton>button {
        border-radius: 4px; font-weight: bold; width: 100%; transition: all 0.2s ease;
    }
    /* Add Button */
...
# --- 2. DATA DICTIONARIES ---
# Hardcoding names prevents API lag on the home page!
TICKER_NAMES = {
...
if 'my_tickers' not in st.session_state:
    st.session_state.my_tickers = list(TICKER_NAMES.keys())

# --- 4. ROUTING & LOGIC FUNCTIONS ---
...
if st.session_state.current_view == 'home':
    ...
    with c1:
        new_ticker = st.text_input("Search ticker (e.g., PLTR, SPY):", label_visibility="collapsed").strip().upper()
    with c2:
    ...
    # Clean, lag-free vertical list
    for ticker in st.session_state.my_tickers:
        # Get the full name, or show (Custom Asset) if it's one they added manually
        full_name = TICKER_NAMES.get(ticker, "Custom Asset")
        
        # 4 Columns: Ticker, Name, View Button, Remove Button
        col1, col2, col3, col4 = st.columns([1, 4, 1.5, 1])
        
        with col1:
            st.markdown(f"**{ticker}**")
        with col2:
            st.markdown(f"<span style='color: #94a3b8;'>({full_name})</span>", unsafe_allow_html=True)
        with col3:
            st.button("🔍 View", key=f"view_{ticker}", type="primary", on_click=go_to_detail, args=(ticker,))
        with col4:
            st.button("❌", key=f"rem_{ticker}", on_click=remove_ticker, args=(ticker,))
...
# --- 6. THE DETAIL PAGE ---
    ...
    c1, c2 = st.columns([4, 1])
    with c1:
        st.title(f"🔍 PROFILE: {title_display}")
    with c2:
